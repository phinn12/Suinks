...
resolvePackages_fn = async function(packages) {
  ...
  const batches = chunk2(packages, __privateGet3(this, _pageSize));
  const results = {};
  await Promise.all(
...
resolveTypes_fn = async function(types) {
  ...
  const batches = chunk2(types, __privateGet3(this, _pageSize));
  const results = {};
  await Promise.all(
    batches.map(async (batch) => {
      var _a;
...
export {
  ...
  toBase642,
  chunk2 as chunk,
  promiseWithResolvers2 as promiseWithResolvers,
...
//# sourceMappingURL=chunk-N5GBBSG5.js.map
